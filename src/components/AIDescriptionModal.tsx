...
export const AIDescriptionModal = ({ isOpen, onClose, originalText, generatedText, onReplace, isLoading }: AIDescriptionModalProps) => {
  ...
  {/* Add min-h-0 here so children can scroll */}
  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-grow min-h-0">
    <div className="flex flex-col min-h-0">
      <h3 className="font-semibold mb-2">Your Text</h3>
      <ScrollArea className="bg-black/20 rounded-md p-4 border border-white/10 flex-1 min-h-0">
    ...
    <div className="flex flex-col min-h-0">
      ...
      <ScrollArea className="bg-black/20 rounded-md p-4 border border-amber-400/30 flex-1 min-h-0">
        {isLoading ? (
